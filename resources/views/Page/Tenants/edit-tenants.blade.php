        ...
        <form method="POST" action="{{ route('tenants.update', $tenant->id) }}">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('lastName') is-invalid @enderror" id="lastName"
                    name="lastName" value="{{ old('lastName', $tenant->lastName) }}" required>
                @error('lastName')
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('firstName') is-invalid @enderror" id="firstName"
                    name="firstName" value="{{ old('firstName', $tenant->firstName) }}" required>
                @error('firstName')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('adress') is-invalid @enderror" id="adress" name="adress"
                    value="{{ old('adress', $tenant->adress) }}" required>
                @error('adress')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('status') is-invalid @enderror" id="status" name="status">
                    <option value="actif" {{ old('status', $tenant->status) == 'actif' ? 'selected' : '' }}>Actif</option>
                    <option value="inactif" {{ old('status', $tenant->status) == 'inactif' ? 'selected' : '' }}>Inactif
                    </option>
                    <option value="resilié" {{ old('status', $tenant->status) == 'resilié' ? 'selected' : '' }}>Résilié
                    </option>
                </select>
                @error('status')
                    <div class="invalid-feedback">{{ $message }}</div>
